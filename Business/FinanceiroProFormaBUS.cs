...
namespace Business
{
    public class FinanceiroProFormaBUS
    {
        FinanceiroProFormaDAL dal = new FinanceiroProFormaDAL();

        public List<FinanceiroProForma> Filtro(DateTime dataInicioDT, DateTime dataTerminoDT, string codTMV, string numeroMov, string invoice, string documento, string situacao, int id_integracao)
        {
            List<FinanceiroProForma> lst = dal.Filtro(dataInicioDT, dataTerminoDT, codTMV, numeroMov, invoice, documento, situacao, id_integracao);
            return lst;
        }
